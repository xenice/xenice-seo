...
trait Elements
{
    ...
    /*
    private function price($field)
    {
        $input =<<<js
        <label><input type="text" onkeyonup="InputPrice(this);" class="%s-text" name="%s" value="%s" %s/>%s</label>
js;
        return $this->text($field, $input);
    }*/

    private function number($field)
    {
        $style = $field['style']??'small';
        return sprintf( '<label><input type="number" class="%s-text" name="%s" value="%s" step="%s" min="%s" max="%s" />%s</label>', 
        $style,$field['id'], $field['value'], $field['step']??1,$field['min']??0,$field['max']??'',$field['label']??'');
    }
    ...
    {
        $style = $field['style']??'regular';
        $button = sprintf('<button class="button button-default xenice-img-select-%s">%s</button>',$field['id'],xx("Select"));
...
js;
        add_filter('xenice_'.$this->key.'_add_js', function($str)use($js){
            $str .= "\r\n" . $js;
            return $str;
        });
        return $str;
    ...
    private function imgs($field)
    {
        ...
        $str .='</div>';
        $str .= sprintf('<button class="button button-default xenice-img-add-%s">%s</button>',$field['id'],xx("Add"));
...
js;
    ...
    private function slide($field)
    {
        $add_image = function($index, $value = [])use($field){
            ...
            $str .= '<div style="margin-bottom:10px;font-weight:bold;">' . xx('Image') . ' ' . $index .'</div>';
            $str .= '<div class="slide-data"><div style="margin-bottom:5px">' . xx('URL:').'</div>';
            $str .= sprintf( '<input type="text" class="regular-text" id="%s" value="%s" />', $field['id'] . '_url_' . $index, $value['url']??'');
            $str .= '<div style="margin-bottom:5px;margin-top:10px">' . xx('Title:').'</div>';
            $str .= sprintf( '<input type="text" class="regular-text" id="%s" value="%s" />', $field['id'] . 'xxitle_' . $index, $value['title']??'');
            $str .= '<div style="margin-bottom:5px;margin-top:10px">' . xx('Description:').'</div>';
        ...
        $str .= sprintf('</div><button class="button button-default xenice-slide-add-%s">%s</button>', $field['id'], xx('Add'));
